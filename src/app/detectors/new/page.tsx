...
export default function NewDetectorPage() {
  ...
  const handleInputChange = <K extends keyof DetectorForm>(
    field: K,
    value: DetectorForm[K]
  ) => {
    setFormData((prev) => ({
      ...prev,
      [field]: value,
  ...
  const validateForm = (): boolean => {
    if (!formData.detector_id.trim()) {
      toast.error("Detector ID is required");
      return false;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data } = await api.post(apiConfig.endpoints.detectors, payload);
      toast.success("Detector created successfully!");
      router.push(`/detectors/${data.detector_id}`);
    } catch (error) {
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <Label htmlFor="detector_id">Detector Name *</Label>
                    <Input
                      id="detector_id"
                      value={formData.detector_id}
                      onChange={(e) =>
                        handleInputChange("detector_id", e.target.value)
                      }
                      placeholder="my_custom_detector"
                    />
                    <p className="text-xs text-gray-600 mt-1">
                      Unique identifier for the detector (lowercase, underscores
                      allowed)
                    </p>
                  </div>
                  <div>
                  ...
                  <div className="flex items-center space-x-2 pt-6">
                    <Checkbox
                      ...
                      onCheckedChange={(checked) =>
                        handleInputChange("enabled", Boolean(checked))
                      }
                    />
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-2 text-sm">
                  <p>
                    <strong>ID:</strong>{" "}
                    {formData.detector_id || "Not specified"}
                  </p>
                  <p>
                    <strong>Name:</strong> {formData.detector_id || "Not specified"}
                  </p>
                  <p>
            ...
            <div className="flex space-x-4">
              <Button type="submit" disabled={loading} className="flex-1">
                ...
                {loading ? "Creating..." : "Create Detector"}
              </Button>
              <Link href="/detectors">
